...
Estos resultados se suelen resumir en la tabla ANOVA, de esta forma:
\\[10pt]
\begin{tabular}{|l|c|c|c|c|}
\hline
\begin{tabular}[c]{@{}l@{}}Fuente de \\ variaci\'on\end{tabular} & G. de L.     & Suma de Cuadrados                                                                                 & Media                           & F                                                  \\ \hline
Factor  & a-1 & $\displaystyle\sum_{i=1}^an_i(\bar{Y}_{i.}-\bar{Y}_{..})^2$                             & $\displaystyle\dfrac{SCTR}{a-1}$ & $\displaystyle F=\dfrac{SCTR/(a-1)}{SCE/(N-a)}$            \\
Error & N-a & $\displaystyle\sum_{i=1}^a\sum_{j=1}^{n_i}(Y_{ij}-\bar{Y}_{i.})^2$                                   & $\displaystyle\dfrac{SCE}{N-a}$          &                                                    \\ \hline
Total & N-1 & $\displaystyle\sum_{i=1}^a\sum_{j=1}^{n_i}(Y_{ij}-\bar{Y}_{..})^2$                                   &                                 &                                                    \\ \hline
\end{tabular}
...
\hline
\begin{tabular}[c]{@{}l@{}}Fuente de \\ variaci\'on\end{tabular} & G. de L.     & Suma de Cuadrados                                                                                 & Media                           & F                                                  \\ \hline
Factor A                                                         & a-1        & $\displaystyle\sum_{i=1}^a\sum_{j=1}^b\sum_{k=1}^n(\bar{Y}_{i..}-\bar{Y}_{...})^2$                             & $\displaystyle\dfrac{SCTR_A}{a-1}$           & $\displaystyle F_A=\dfrac{SCTR_A/(a-1)}{SCE/ab(n-1)}$            \\
Factor B                                                         & b-1        & $\displaystyle\sum_{i=1}^a\sum_{j=1}^b\sum_{k=1}^n(\bar{Y}_{.j.}-\bar{Y}_{...})^2$                             & $\displaystyle\dfrac{SCTR_B}{b-1}$           & $\displaystyle F_B=\dfrac{SCTR_B/(b-1)}{SCE/ab(n-1)}$            \\
Factor AB                                                        & (a-1)(b-1) & $\displaystyle\sum_{i=1}^a\sum_{j=1}^b\sum_{k=1}^n(\bar{Y}_{ij.}-\bar{Y}_{i..}-\bar{Y}_{.j.}+\bar{Y}_{...})^2$ & $\displaystyle\dfrac{SCTR_{AB}}{(a-1)(b-1)}$ & $\displaystyle F_{AB}=\dfrac{SCTR_{AB}/(a-1)(b-1)}{SCE/ab(n-1)}$ \\
Error                                                            & ab(n-1)    & $\displaystyle\sum_{i=1}^a\sum_{j=1}^b\sum_{k=1}^n(Y_{ijk}-\bar{Y}_{ij.})^2$                                   & $\displaystyle\dfrac{SCE}{ab(n-1)}$          &                                                    \\ \hline
Total                                                            & abn-1      & $\displaystyle\sum_{i=1}^a\sum_{j=1}^b\sum_{k=1}^n(Y_{ijk}-\bar{Y}_{...})^2$                                   &                                 &                                                    \\ \hline
\end{tabular}
%%\end{table}

La regla de decisi\'on para el contraste de la $F$ se basa en el nivel de significaci\'on ($p$-valor):\\
Rechazaremos $H_0^{\prime}$ si $F_A>F_{a-1,ab(n-1);\alpha}$.\\
Rechazaremos $H_0^{\prime\prime}$ si $F_B>F_{b-1,ab(n-1);\alpha}$.\\
Rechazaremos $H_0^{\prime\prime\prime}$ si $F_{AB}>F_{(a-1)(b-1),ab(n-1);\alpha}$.\\

Si rechazamos alguna hip\'otesis nula, hay diferencias entre las medias, y se deben a los factores para los que se rechaza la hip\'otesis.

Si no podemos rechazar ninguna hip\'otesis nula, no podemos afirmar que haya diferencias entre las medias, y por tanto no podemos afirmar que los factores influyan en las mismas.